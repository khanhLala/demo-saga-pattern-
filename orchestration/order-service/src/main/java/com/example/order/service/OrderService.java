...
public class OrderService {
    ...
    public Order createOrder(OrderRequest request) {
        ...
        log.info("Da tao order | txId={} | status=PENDING", order.getTransactionId());

        try {
            // 2. Call Inventory to Reserve
            log.info("Dang goi InventoryService.reserve | txId={}", order.getTransactionId());
            ...
            if (invRes != null && invRes.isSuccess()) {
                log.info("InventoryReserved thanh cong | txId={}", order.getTransactionId());

                // 3. Call Payment to Charge
                log.info("Dang goi PaymentService.charge | txId={}", order.getTransactionId());
                ...
                if (payRes != null && payRes.isSuccess()) {
                    ...
                    log.info("Payment charge thanh cong | txId={}", order.getTransactionId());
                    order.setStatus(OrderStatus.COMPLETED);
                } else {
                    // 5a. Payment failed -> Rollback Inventory and Update Order FAILED
                    log.warn("Payment charge THAT BAI | txId={} | reason={}", 
                ...
                log.warn("Inventory reserve THAT BAI | txId={} | reason={}", 
                        order.getTransactionId(), invRes != null ? invRes.getMessage() : "null");
                order.setStatus(OrderStatus.FAILED);
            }
        } catch (Exception e) {
            log.error("Loi trong qua trinh orchestration | txId={} | logic={}", order.getTransactionId(), e.getMessage());
            order.setStatus(OrderStatus.FAILED);
        }

        orderRepository.save(order);
        log.info("Hoan tat workflow | txId={} | status={}", order.getTransactionId(), order.getStatus());
        return order;
    }

    private void rollbackInventory(String transactionId) {
        try {
            log.info("Dang thuc hien compensation: Refund Inventory | txId={}", transactionId);
            ...
            log.error("Loi khi rollback inventory | txId={} | error={}", transactionId, e.getMessage());
        }
    }
}
